...
module SquareGeom where

open import Spartan
open import CatAxioms
open import HigherCat using (𝕀; 𝕀-ob; 𝟚)
open import Constructions using (dom; 𝕀₁; !)
open import Pullbacks using (_×c_; π₁; π₂; ⟨_,_⟩; pair-β₁; pair-β₂; pair-η; equiv-inv-rinv)
open import Product using (product-map; injl; injr; injl-product-map; injr-product-map; ipe)
open import Exponentials using (Fun)
open import Interval using (Δ; Ord)
open import FaceMap using (face)
open import FaceTop using (top; c20-top)
open import LastEdge using (last-edge)
open import UnitIso using (eval-pt)
open import PushoutBase using (eval-pt-dom-last)
open import FaceTopProof using (face-top)
open import SimplexRetract using (sect; ret; ret-sect; pre-face-n; eval-at-top)
open import SimplexRetractValues using
...
open import OrdComb using (sn2-pr1; sn2-snd; sn2-pr1-inl; sn2-snd-face;
  thr-ord; thr-dnilast; thr-nonbot; ord-suc-top; nbot-top)
open import FaceOrdCompat using (c20-face-gen-u)
open import Posetal using (posetal-eq-objects; Delta-posetal; I-posetal)
open import Square
