...
open import Category.Constructions using (Hom; Hom-map; dom; cod; id-mor; dom-id; cod-id;
  𝕀₀; 𝕀₁; dom-nat; cod-nat; funct-on-mor; equiv-inv)
open import Category.Pullbacks using (_×c_; π₁; π₂; ⟨_,_⟩; pair-β₁; pair-β₂; pair-η;
  equiv-inv-rinv; equiv-inv-linv)
open import Category.Exponentials using (Fun; ev; exp-comparison; exp-equiv;
  ob-to-map; map-to-ob; ob-map-roundtrip₁; ob-map-roundtrip₂)
open import Interval.Interval using (Δ; ob-to-𝟚; 𝕀-char; 𝕀-char-comparison;
...
open import Foundations.HLevels using (hedberg-set; ob-𝕀-is-set; ×-is-prop;
  fn-into-contr-is-contr; Σ-prop-contr; equiv-is-set; fn-into-set;
  Σ-set-prop; 𝟚-is-set)
open import Interval.IntervalPosetal using (𝕀-thin; 𝕀-hom-to-order; map-into-𝕀-eq; val-𝕀;
  𝕀-order-to-hom)
open import Interval.FaceOrdCompat using (ev-point)
