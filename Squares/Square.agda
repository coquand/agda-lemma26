...
module Squares.Square where

open import Foundations.Spartan
open import Category.CatAxioms
open import Category.HigherCat using (𝕀)
open import Category.Pullbacks using (_×c_)
open import Category.Product using (product-map; pmc; product-map-eq1)
...
-- the correct notion of square morphism in general.  It suffices here only because
-- the squares this is actually applied to (the gluing squares `Sq n`) have posetal
-- corners (thin / is-set homs) — so any two such 2-cells are equal and the missing
-- coherence holds automatically.  This would NOT be sound for arbitrary corners.
